...
component extends="coldbox.system.EventHandler" {
	...
	function aroundHandler(
		...
		// Validation Exceptions
			...
			log.error(
				...
				{ "_stacktrace": e.stacktrace, "httpData": getHTTPRequestData() }
			);
		...
		if (
			...
			if ( prc.response.getError() ) {
				responseData = prc.response.getDataPacket();
			}
			// Magical renderings
			event.renderData(
				type = prc.response.getFormat(),
	...
	function onError(
		...
		log.error(
			...
			{ "_stacktrace": arguments.exception.stacktrace, "httpData": getHTTPRequestData() }
		);
		...
		if ( getSetting( "environment" ) neq "development2" ) {
			// Render Error Out
			event.renderData(
				type = prc.response.getFormat(),
				data = prc.response.getDataPacket(),
	...
	function onValidationException(
		...
		var allErrors = deserializeJSON( exception.extendedInfo ).reduce( function( allErrors, field, fieldErrors ) {
			arrayAppend(
				allErrors,
		...
		prc.response.setErrorMessage( allErrors, 412 );
	...
	function onAuthorizationFailure(
		...
		if ( arguments.abort ) {
			...
			writeOutput( serializeJSON( prc.response.getDataPacket() ) );
